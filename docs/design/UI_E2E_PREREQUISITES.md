...
| ステップ | 操作 | 期待 |
|---------|------|------|
| 1 | `ImportService.upload_csv(file, store)` | `CsvImport` + 複数 `CsvImportRow` 作成。各行 `status="validated"` |
| 2 | `MatchingService.run_matching(csv_import, store)` | 各行に候補が紐づく。`status` が `"pending_review"` / `"confirmed"`（自動マッチ）等に更新 |
| 3 | `MatchingService.get_candidates(row, store)` | Visit 候補リストが返る（`visit_id`, `customer`, `name_match_score` 付き） |
| 4 | `MatchingService.confirm_row(row, visit_id, store)` | 行の `status="confirmed"`。`CsvImportRow.matched_visit` FK が設定される（Visit の新規作成はしない） |
| 5 | `MatchingService.reject_row(row, store)` | 行の `status="rejected"` |
| 6 | 確定された行の Customer の `visit_count` | Visit は matching 以前に作成済み。confirm は FK 紐づけのみのため、visit_count / segment は confirm 前後で変化しないこと |

> **状態名の対応**: 本要求書の初版では `pending/matched/no_match` と記載していたが、headless 実装上の正確な状態名は `validated/pending_review/confirmed`。

**UI への影響**: CSV アップロード → 行一覧 → マッチング実行 → 候補表示 → 確定/却下の全画面遷移。`confirm_row` は Visit を作成せず CsvImportRow の FK と status を更新するのみ。Visit 作成時に発火済みの signal chain（F-03）が confirm 操作で壊れないことが headless E2E で確認済み。
...
- [2026-04-02] Headless E2E テスト結果を受けて修正（v3）
  - F-04: 状態名を headless 実装に合わせて修正（`pending` → `validated`, `matched` → `pending_review`）
  - F-04: `confirm_row` の動作を正確に記載（Visit 新規作成ではなく CsvImportRow.matched_visit FK 更新のみ）
  - F-04 ステップ 6: signal chain の検証方針を「confirm 前後で visit_count/segment が壊れないこと」に修正
  - Headless E2E 結果: F-01〜F-08 全 46 テスト PASS。プロダクションコード変更なし。`QRAuthService.issue_token()` は headless に不在（UI フォールバック前提）
