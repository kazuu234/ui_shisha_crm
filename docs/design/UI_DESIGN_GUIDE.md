...
- **使用禁止**: テーブルヘッダー（情報のカテゴリ表示）、ナビゲーションラベル、エラーメッセージ
- テーブルヘッダー・ナビ非アクティブには `--text-secondary`（6.8:1、WCAG AA 合格）を使うこと
- 12px 以下のテキストには `--text-muted` を使わない（小サイズ × 低コントラストは視認不可）
...
| **disabled** | `--bg-inset` bg, `--text-muted` text | `--bg-inset` bg, `--text-muted` text | `--text-muted` text, `opacity: 0.5` | `--text-muted` text, `opacity: 0.5` |
